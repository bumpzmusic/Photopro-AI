...
import { Download, Camera, Sparkles, Wand2, X, LogOut, Loader2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const fetchBalance = async (userId: string) => {
    try {
      ...
      if (error) {
        ...
        // If row doesn't exist yet (race condition with trigger), default to 5 or handle gracefully
        setTokenBalance(5);
        return;
      }
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        if (error) throw error;
        // For signup, we might want to show a message or auto-login
        if (authMode === 'signup') {
             // Optional: immediately sign in or wait for email confirmation depends on Supabase settings
             // By default Supabase auto-confirms if disabled, or sends email.
             // We'll assume successful signup leads to session or "check email"
        }
      }
    } catch (error: any) {
  ...
  const handleGenerate = async () => {
    if (!currentItem || tokenBalance === null) return;
    ...
    try {
      ...
      // Persist deduction to DB
      await updateBalance(newBalance);

    } catch (e: any) {
      console.error(e);
      // Refund token on error
      setTokenBalance(oldBalance);
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center transition-colors duration-200 ${c.bgApp}`}>
        <div className={`w-full max-w-[400px] p-10 rounded-3xl shadow-2xl border text-center ${c.bgPanel} ${c.border} ${c.text}`}>
           ...
           <h1 className="text-2xl font-bold mb-2">{authMode === 'login' ? t.login_welcome : "Crie sua conta"}</h1>
           ...
           <form onSubmit={handleAuth} className="space-y-4">
              ...
              <Button type="submit" className="w-full h-12 mt-4" disabled={authLoading}>
                {authLoading ? <Loader2 className="animate-spin" /> : (authMode === 'login' ? t.login_btn : "Criar Conta")}
              </Button>
           </form>
        ...
        <div className="p-5 pb-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
               ...
               <div>
                 ...
                 <p className={`text-xs ${c.textSecondary} truncate max-w-[150px]`}>
          ...
          <div className="mt-3 flex justify-between items-center">
             <button 
               ...
               className={`text-xs px-3 py-1.5 rounded border transition-colors flex items-center gap-1 ${c.border} ${c.textSecondary} hover:text-red-500 hover:border-red-500`}
             >
        ...
        <div className="flex justify-end mb-4">
          <div className={`flex flex-col items-end px-2`}>
            ...
            <div className={`text-2xl font-bold ${c.text} flex items-center gap-2 leading-none`}>
              {tokenBalance !== null ? tokenBalance : '...'} <span className="text-xl">🪙</span>
            </div>
          </div>
        </div>
